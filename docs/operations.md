...
```
firmware/      瘦网关固件（firmware.ino + 可原生测试的 modem_* 核心）
...
| `test_firmware_core` | 固件核心宿主编译测试：串口超长行恢复、CNMI/MIPCALL 响应解析、长短信状态机、modem_io_core 命令/队列/URC |
| `test_identity_sanitize` | IMSI/ICCID 脱敏全链路：snapshot 清洗、API 防御过滤、DB 迁移（17 测试） |
| `test_extractor` | 验证码提取真实短信语料（含连字符边界） |
| `test_main_contacts` | 消息含联系人别名 / 搜索、发送号码归一化 |
| `test_keepalive` | 保号使用 MIPCALL 建连/关闭，Ping 异常仍关闭数据连接 |
...
- **收信**：普通短信、验证码短信（提取 `code`）、长短信合并；长短信只收到首段时重启设备，确认分段仍从 SIM 存储恢复且收齐后才删除；webhook 断开后轮询补拉；Hub 重启按游标续拉；设备重启后 NVS 恢复最近短信。
- **通知**：通知送达（Telegram / 各推送渠道）；特殊字符短信不失败；黑名单短信入库但不通知；通知失败后重试。
- **发送**：Web UI 单段 / 多段；Telegram `/sms`；发送失败写入发件记录；管理员短信代发（`收件人:内容`）。
- **管理**：单条/批量删除不回流；清 NVS / 换机后 `gw_epoch` 自愈；AT 控制台 `ATI`/`AT+CESQ`；设备忙时 AT 返回 `409` 且不阻塞 poller；Docker volume 备份可恢复。
- **接收自愈**：状态页「收信配置」显示正常；手工让模组复位并出现 `+MATREADY`/`+CPIN: READY`，确认固件重设 `CMGF/CPMS/CNMI` 且恢复计数增加。
- **保号（可选）**：启用后到期执行 `MIPCALL=1 → MPING → MIPCALL=0`，结果推送通知；结束后状态页数据连接为关闭。
...
# 上传（替换 COM7 为实际端口）
arduino-cli upload --fqbn esp32:esp32:esp32c3:PartitionScheme=huge_app \
  --port COM7 ./firmware/firmware.ino
```

Arduino IDE 关键选项：开发板 `MakerGO ESP32 C3 SuperMini`（没有就选 `ESP32C3 Dev Module`，但分区方案必须对）、`Partition Scheme: Huge APP (3MB No OTA/1MB SPIFFS)`、`Upload Speed: 460800`。上传失败可降到 `115200`，或手动进下载模式（按住 `BOOT`，点按 `RST`，上传开始后松开 `BOOT`）。
...
本项目使用 `Huge APP` 分区方案，不支持 OTA 无线升级。所有固件更新均需 USB 有线刷写，重复上述编译与上传步骤即可。
