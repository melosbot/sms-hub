...
> 本文是系统设计、数据模型、HTTP 接口契约的**单一权威来源**。运维操作见 [operations.md](operations.md)；机器可读契约见 [`openapi.yaml`](openapi.yaml)；固件可靠性路线图见 [../TODO.md](../TODO.md)。
>
> v2 是**全新安装**（`SCHEMA_VERSION=4`，不兼容旧库）。支持**多台瘦终端 + 多张 SIM 卡**接入同一个 Hub。
