...
// 只做短信收发与缓存,HTTP API 形如 /<token>/(status|pull|send|at|delete),
...
#define MAX_PENDING_STORED_DELETES 32
#define MAX_STORED_SMS_READ_ATTEMPTS 5
#define READ_RETRY_BASE_MS 15000UL
#define READ_RETRY_MAX_MS 900000UL

#define SMS_RX_STATUS_QUERY_MS 30000UL
#define SMS_RX_CONFIG_REFRESH_MS 300000UL
...
PDU pdu = PDU(4096);
WebServer server(80);
ModemLineReader modemLineReader;
static_assert(MODEM_LINE_BUFFER_SIZE == ModemLineReader::kCapacity,
              "modem line buffer capacities must match");

// ───────────────────────── 运行状态 ─────────────────────────
bool modemCommandActive = false;
String activeModemCommand = "";
...
bool cachedDataConnectionActive = false;  // ML307 MIPCALL 应用层数据连接状态
...
enum class SmsRxRecoveryStep : uint8_t {
  Idle,
  PduMode,
  Storage,
  NotifyMode,
...
SmsRxRecoveryStep smsRxRecoveryStep = SmsRxRecoveryStep::Idle;
bool smsRxConfigKnown = false;
bool smsRxConfigOk = false;
unsigned long smsRxNextActionMs = 0;
unsigned long smsRxNextStatusMs = 0;
unsigned long smsRxNextRefreshMs = 0;
unsigned long smsRxLastCheckMs = 0;
...
unsigned long dataGuardNextAttemptMs = 0;
uint32_t dataGuardFailureCount = 0;
// 删除队列统计
uint32_t deleteFailTotal = 0;
...
struct PendingStoredDelete {
  bool inUse;
  int index;
  uint8_t attempts;
  unsigned long nextAttempt;
};
PendingStoredDelete pendingStoredDeletes[MAX_PENDING_STORED_DELETES];
...
enum class PduProcessResult {
  Rejected,
  Stored,
  ConcatPending,
  ConcatFinalized,
};
PduProcessResult processPduLine(const String& pduLine, int storageIndex = 0);
bool handleModemUrcLine(String& line);
String assembleConcatSms(int slot);
void finalizeConcatSlot(int slot, bool complete);
bool isHexString(const String& str);
bool isMmsNotificationPdu(const String& pduLine);
bool enqueueStoredSmsIndex(int index);
bool enqueueStoredSmsDelete(int index);
bool isConcatStoredIndexPending(int index);
void scheduleSmsRxRecovery(const char* reason);
void maintainSmsRxConfig();
bool refreshDataConnectionStatus();
bool ensureDataConnectionClosed();
void maintainDataConnectionGuard();
...
bool readModemLine(String& out) {
  while (Serial1.available()) {
    char c = Serial1.read();
    ModemLineResult result = modemLineReader.onByte(c);
    if (result == ModemLineResult::Dropped) {
      Serial.println("模组响应行过长,已丢弃整行");
      continue;
    }
    if (result == ModemLineResult::Line) {
      out = String(modemLineReader.line());
      out.trim();
      return true;
    }
  }
...
String sendATCommand(const char* cmd, unsigned long timeout) {
  ...
  String previousCommand = activeModemCommand;
  modemCommandActive = true;
  activeModemCommand = String(cmd);
  ...
  while (millis() - start < timeout) {
    ...
    if (readModemLine(line)) {
      ...
      if (isFinalAtResponseLine(line)) {
        ...
        activeModemCommand = previousCommand;
        return resp;
      }
    } else {
      delay(2);
...
bool atResponseHasOk(const String& response) {
  return modem_policy::hasOkLine(response.c_str());
}

bool smsRxConfigResponseMatches(const String& response) {
  return modem_policy::cnmiUsesStorageNotifications(response.c_str());
}

void scheduleSmsRxRecovery(const char* reason, uint8_t reasonCode = 0) {
  smsRxRecoveryStep = SmsRxRecoveryStep::PduMode;
  smsRxNextActionMs = millis();
  smsRxConfigKnown = false;
  smsRxConfigOk = false;
  if (reasonCode) smsRxLastRecoveryReason = reasonCode;
  Serial.printf("短信接收配置待恢复: %s\n", reason ? reason : "unknown");
}

void maintainSmsRxConfig() {
  if (!modemInitDone || modemCommandActive) return;

  unsigned long now = millis();
  if (smsRxRecoveryStep == SmsRxRecoveryStep::Idle) {
    if (timeReached(now, smsRxNextRefreshMs)) {
      scheduleSmsRxRecovery("periodic_refresh", 4);
      return;
    }
    if (!timeReached(now, smsRxNextStatusMs)) return;

    String response = sendATCommand("AT+CNMI?", 2000);
    smsRxLastCheckMs = millis();
    smsRxNextStatusMs = smsRxLastCheckMs + SMS_RX_STATUS_QUERY_MS;
    smsRxConfigKnown = atResponseHasOk(response);
    smsRxConfigOk = smsRxConfigKnown && smsRxConfigResponseMatches(response);
    if (!smsRxConfigOk) scheduleSmsRxRecovery("CNMI mismatch or query failed", 3);
    return;
  }

  if (!timeReached(now, smsRxNextActionMs)) return;

  const SmsRxRecoveryStep executing = smsRxRecoveryStep;
  bool ok = false;
  switch (executing) {
    ...
    case SmsRxRecoveryStep::NotifyMode:
      ...
      break;
    case SmsRxRecoveryStep::Verify: {
      String response = sendATCommand("AT+CNMI?", 2000);
      smsRxLastCheckMs = millis();
      smsRxConfigKnown = atResponseHasOk(response);
      ok = smsRxConfigKnown && smsRxConfigResponseMatches(response);
      break;
    }
    case SmsRxRecoveryStep::Idle:
      return;
  }

  now = millis();
  if (smsRxRecoveryStep != executing) {
    // 执行期间收到模组重启类 URC，保留事件触发的从头恢复请求。
  ...
  if (executing == SmsRxRecoveryStep::PduMode) {
    smsRxRecoveryStep = SmsRxRecoveryStep::Storage;
  } else if (executing == SmsRxRecoveryStep::Storage) {
    smsRxRecoveryStep = SmsRxRecoveryStep::NotifyMode;
  } else if (executing == SmsRxRecoveryStep::NotifyMode) {
    smsRxRecoveryStep = SmsRxRecoveryStep::Verify;
  } else {
    smsRxRecoveryStep = SmsRxRecoveryStep::Idle;
    smsRxConfigKnown = true;
    smsRxConfigOk = true;
    ...
    Serial.printf("短信接收配置恢复完成(累计 %lu 次)\n",
                  (unsigned long)smsRxRecoveryCount);
  }
  smsRxNextActionMs = now;
}

bool parseMipCallResponse(const String& response, bool& active) {
  return modem_policy::parseMipCallActive(response.c_str(), active);
}

bool refreshDataConnectionStatus() {
  String response = sendATCommand("AT+MIPCALL?", 3000);
  bool active = false;
  if (!parseMipCallResponse(response, active)) return false;
  cachedDataConnectionActive = active;
  return true;
}

bool ensureDataConnectionClosed() {
  if (!refreshDataConnectionStatus()) return false;
  if (!cachedDataConnectionActive) return true;

  if (!sendATandWaitOK("AT+MIPCALL=0,1", 5000)) return false;
  cachedDataConnectionActive = false;
  return true;
}

void maintainDataConnectionGuard() {
  if (!modemInitDone || modemCommandActive || dataGuardSatisfied) return;
  unsigned long now = millis();
  if (!timeReached(now, dataGuardNextAttemptMs)) return;

  dataGuardSatisfied = ensureDataConnectionClosed();
  if (dataGuardSatisfied) {
    Serial.println("MIPCALL 应用数据连接已关闭");
  } else {
    dataGuardFailureCount++;
    dataGuardNextAttemptMs = millis() + 60000UL;
    recordLastError("关闭 MIPCALL 数据连接失败");
  }
}
...
void refreshModemStatus(bool force) {
  ...
  // 飞行模式 / MIPCALL 应用数据连接:随 60s 刷新一并更新,UI 免 AT 读取
...
bool enqueueStoredSmsDelete(int index) {
  ...
  for (int i = 0; i < MAX_PENDING_STORED_DELETES; i++) {
    if (pendingStoredDeletes[i].inUse && pendingStoredDeletes[i].index == index) return true;
  }
  for (int i = 0; i < MAX_PENDING_STORED_DELETES; i++) {
    if (!pendingStoredDeletes[i].inUse) {
      pendingStoredDeletes[i].inUse = true;
      pendingStoredDeletes[i].index = index;
      pendingStoredDeletes[i].attempts = 0;
      pendingStoredDeletes[i].nextAttempt = millis();
      return true;
    }
  }

  Serial.printf("短信删除队列已满,保留模组存储 #%d\n", index);
  recordLastError("短信删除队列已满");
  return false;
}

void processPendingStoredDeletes() {
  if (modemCommandActive) return;

  unsigned long now = millis();
  for (int i = 0; i < MAX_PENDING_STORED_DELETES; i++) {
    PendingStoredDelete& pending = pendingStoredDeletes[i];
    if (!pending.inUse || !timeReached(now, pending.nextAttempt)) continue;

    String cmd = "AT+CMGD=" + String(pending.index);
    if (sendATandWaitOK(cmd.c_str(), 3000)) {
      Serial.printf("已删除模组存储短信 #%d\n", pending.index);
      pending = {};
    } else {
      if (pending.attempts < 255) pending.attempts++;
      pending.nextAttempt = millis() + retryDelayForAttempt(pending.attempts);
      Serial.printf("删除模组存储 #%d 失败,稍后重试(%u)\n",
                    pending.index, (unsigned)pending.attempts);
...
bool readStoredSmsAtIndex(int index, bool forwardRawOnFailure) {
  ...
  PduProcessResult result = PduProcessResult::Rejected;
  String pduLineSeen = "";
  ...
  while (start <= (int)resp.length()) {
    ...
    if (line.length() > 0 && line.length() <= MAX_PDU_LENGTH && isHexString(line)) {
      pduLineSeen = line;
      result = processPduLine(line, index);
      break;
    }
    start = end + 1;
  }
  ...
  if (result == PduProcessResult::Rejected && forwardRawOnFailure && pduLineSeen.length() > 0) {
    String text = "[PDU解码失败,原始数据]\n" + pduLineSeen;
    processSmsContent("解码失败", text.c_str(), "", true);
    result = PduProcessResult::Stored;
  }

  if (result == PduProcessResult::Stored) {
    enqueueStoredSmsDelete(index);
  }

  return result != PduProcessResult::Rejected;
}
    ...
    while (start <= (int)resp.length()) {
      ...
      if (line.startsWith("+CMGL:")) {
        ...
        if (colonIdx >= 0 && commaIdx > colonIdx) {
          ...
          if (idx > 0 && idx < minIndex && !isConcatStoredIndexPending(idx)) minIndex = idx;
        }
      }
      start = end + 1;
    }
...
void initConcatBuffer() {
  for (int i = 0; i < MAX_CONCAT_MESSAGES; i++) {
    ...
    for (int j = 0; j < MAX_CONCAT_PARTS; j++) {
      ...
      concatBuffer[i].parts[j].storageIndex = 0;
    }
  }
}
...
int findOrCreateConcatSlot(int refNumber, const char* sender, int totalParts) {
  ...
  if (concatBuffer[oldestSlot].receivedParts > 0) {
    finalizeConcatSlot(oldestSlot, false);
  }
  ...
  for (int j = 0; j < MAX_CONCAT_PARTS; j++) {
    ...
    concatBuffer[oldestSlot].parts[j].storageIndex = 0;
  }
  return oldestSlot;
}
...
void clearConcatSlot(int slot) {
  ...
  for (int j = 0; j < MAX_CONCAT_PARTS; j++) {
    ...
    concatBuffer[slot].parts[j].storageIndex = 0;
  }
}
...
bool isConcatStoredIndexPending(int index) {
  if (index <= 0) return false;
  for (int i = 0; i < MAX_CONCAT_MESSAGES; i++) {
    if (!concatBuffer[i].inUse) continue;
    for (int j = 0; j < MAX_CONCAT_PARTS; j++) {
      if (concatBuffer[i].parts[j].valid &&
          concatBuffer[i].parts[j].storageIndex == index) return true;
    }
  }
  return false;
}
...
void finalizeConcatSlot(int slot, bool complete) {
  ...
  String text = assembleConcatSms(slot);
  processSmsContent(concatBuffer[slot].sender.c_str(),
                    text.c_str(),
                    concatBuffer[slot].timestamp.c_str(),
                    complete);
  for (int i = 0; i < MAX_CONCAT_PARTS; i++) {
    if (concatBuffer[slot].parts[i].valid) {
      enqueueStoredSmsDelete(concatBuffer[slot].parts[i].storageIndex);
    }
  }
...
PduProcessResult processPduLine(const String& pduLine, int storageIndex) {
  ...
  if (isMmsNotificationPdu(pduLine)) {
    ...
    return PduProcessResult::Stored;
  }
  ...
  if (totalParts > 1 && partNumber > 0) {
    ...
    if (partIndex >= 0 && partIndex < MAX_CONCAT_PARTS) {
      if (!concatBuffer[slot].parts[partIndex].valid) {
        ...
        concatBuffer[slot].parts[partIndex].storageIndex = storageIndex;
        ...
        if (concatBuffer[slot].parts[partIndex].storageIndex == 0) {
          // 先从 +CMT 收到、随后又落入模组存储时，补记可跨重启恢复的索引。
          concatBuffer[slot].parts[partIndex].storageIndex = storageIndex;
        } else if (concatBuffer[slot].parts[partIndex].storageIndex != storageIndex) {
          // 同一分段被重复存储；首份仍作为恢复源，重复索引可安全删除。
          enqueueStoredSmsDelete(storageIndex);
        }
      }
    } else {
      recordLastError("长短信分段编号超过缓存上限");
      return PduProcessResult::Rejected;
    }

    if (concatBuffer[slot].receivedParts >= totalParts) {
      Serial.println("长短信已收齐,合并入缓冲");
      finalizeConcatSlot(slot, true);
      return PduProcessResult::ConcatFinalized;
    }
    return PduProcessResult::ConcatPending;
  }
...
bool handleModemUrcLine(String& line) {
  ...
  if (line == "+MATREADY" ||
      (line.startsWith("+CPIN: READY") && activeModemCommand != "AT+CPIN?")) {
    if (modemInitDone) {
      uint8_t reasonCode = (line == "+MATREADY") ? 1 : 2;
      scheduleSmsRxRecovery(line.c_str(), reasonCode);
    }
    return true;
  }
...
void maintainHeartbeat() {
  ...
  body += "\"ip\":\"" + ip + "\",\"port\":80,";
  body += "\"fw\":\"" FW_VERSION "\",";
  appendf(body, "\"protocol_version\":%d,", DEVICE_PROTOCOL_VERSION);
  appendf(body, "\"capabilities\":%lu,", (unsigned long)DEVICE_CAPABILITIES);
  ...
  // pdp_active 为旧 Hub/Web UI(≤v2.0.x)兼容别名,将在 v2.2 移除；语义已切换为 MIPCALL 应用连接。
  body += "\"pdp_active\":" + boolJson(cachedDataConnectionActive) + ",";
  body += "\"data_connection_active\":" + boolJson(cachedDataConnectionActive) + ",";
  body += "\"data_guard_ok\":" + boolJson(dataGuardSatisfied) + ",";
  appendf(body, "\"data_guard_failures\":%lu,", dataGuardFailureCount);
  body += "\"sms_rx_config_known\":" + boolJson(smsRxConfigKnown) + ",";
  body += "\"sms_rx_config_ok\":" + boolJson(smsRxConfigOk) + ",";
  appendf(body, "\"sms_rx_last_check_ms\":%lu,", smsRxLastCheckMs);
  appendf(body, "\"sms_rx_last_config_ms\":%lu,", smsRxLastConfigMs);
  appendf(body, "\"sms_rx_recoveries\":%lu,", smsRxRecoveryCount);
  appendf(body, "\"sms_rx_failures\":%lu,", smsRxFailureCount);
  appendf(body, "\"sms_rx_last_recovery_reason\":%u", smsRxLastRecoveryReason);
  body += "},";
  // 缓冲状态
  body += "\"buffer\":{";
  ...
  // 删除队列
  body += "\"delete_queue\":{";
  { int dqDepth = 0; unsigned long dqOldestMs = 0;
    unsigned long nowMs = millis();
    for (int i = 0; i < MAX_PENDING_STORED_DELETES; i++) {
      if (pendingStoredDeletes[i].inUse) {
        dqDepth++;
        unsigned long age = nowMs - pendingStoredDeletes[i].nextAttempt;
        if (age > dqOldestMs) dqOldestMs = age;
      }
    }
    appendf(body, "\"depth\":%d,", dqDepth);
    appendf(body, "\"oldest_age_ms\":%lu,", dqOldestMs);
    appendf(body, "\"failures\":%lu", deleteFailTotal);
  }
  body += "}";
  body += "}";
...
void handleStatus() {
  ...
  json.reserve(768);
  json += "\"fw\":\"" FW_VERSION "\",";
  appendf(json, "\"protocol_version\":%d,", DEVICE_PROTOCOL_VERSION);
  appendf(json, "\"capabilities\":%lu,", (unsigned long)DEVICE_CAPABILITIES);
  ...
  // pdp_active 为旧 Hub/Web UI(≤v2.0.x)兼容别名,将在 v2.2 移除。
  json += "\"pdp_active\":" + boolJson(cachedDataConnectionActive) + ",";
  json += "\"data_connection_active\":" + boolJson(cachedDataConnectionActive) + ",";
  json += "\"data_guard_ok\":" + boolJson(dataGuardSatisfied) + ",";
  appendf(json, "\"data_guard_failures\":%lu,", dataGuardFailureCount);
  json += "\"sms_rx_config_known\":" + boolJson(smsRxConfigKnown) + ",";
  json += "\"sms_rx_config_ok\":" + boolJson(smsRxConfigOk) + ",";
  appendf(json, "\"sms_rx_last_check_ms\":%lu,", smsRxLastCheckMs);
  appendf(json, "\"sms_rx_last_config_ms\":%lu,", smsRxLastConfigMs);
  appendf(json, "\"sms_rx_recoveries\":%lu,", smsRxRecoveryCount);
  appendf(json, "\"sms_rx_failures\":%lu,", smsRxFailureCount);
  appendf(json, "\"sms_rx_last_recovery_reason\":%u", smsRxLastRecoveryReason);
  json += "},";
  ...
  { int dqDepth = 0; unsigned long dqOldestMs = 0;
    ...
    appendf(json, "\"depth\":%d,", dqDepth);
    appendf(json, "\"oldest_age_ms\":%lu,", dqOldestMs);
    appendf(json, "\"failures\":%lu", deleteFailTotal);
  }
  json += "},";
...
void handleAt() {
  ...
  // 飞行模式 / MIPCALL 数据连接被 UI 主动切换后,立即更新缓存并强制补发心跳,
  ...
  if (ok) {
    ...
    } else if (up.startsWith("AT+MIPCALL=")) {
      cachedDataConnectionActive = up.substring(11).startsWith("1");
      heartbeatForce = true;
    }
  }
}
...
void setup() {
  ...
  modemInitDone = true;
  unsigned long smsRxStartedMs = millis();
  smsRxConfigKnown = pduModeOk && storageOk && cnmiOk;
  smsRxConfigOk = smsRxConfigKnown;
  smsRxLastConfigMs = smsRxConfigOk ? smsRxStartedMs : 0;
  smsRxNextStatusMs = smsRxStartedMs + SMS_RX_STATUS_QUERY_MS;
  smsRxNextRefreshMs = smsRxStartedMs + SMS_RX_CONFIG_REFRESH_MS;
  smsRxNextActionMs = smsRxStartedMs;
  smsRxRecoveryStep = smsRxConfigOk ? SmsRxRecoveryStep::Idle : SmsRxRecoveryStep::PduMode;
